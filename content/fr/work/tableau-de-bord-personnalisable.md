...
Trois problèmes ont façonné les décisions les plus importantes.
...
Les tests avec des utilisateurs internes ont globalement validé la direction. Une exception : l'icône indiquant qu'un widget pouvait être déplacé n'était pas lisible. Nous l'avons mise à jour et le problème s'est résolu. Les autres états d'interaction et affordances ont résisté aux tests sans modifications.
...
Le tableau de bord a également introduit un nouveau modèle de filtre. Le tableau de bord statique original disposait d'un filtre global appliqué sur toute la vue. Nous avons pris la décision délibérée de le supprimer en faveur de l'indépendance au niveau des widgets, en gardant uniquement les filtres de sentiment comme globaux. Chaque widget pouvait avoir ses propres filtres, locaux et éphémères, avec la possibilité de les promouvoir au niveau global. Le compromis était intentionnel : l'indépendance des widgets avait plus de valeur à ce stade que le filtrage inter-tableau de bord. Cela a été signalé tôt par l'équipe de développement et conçu en amont.
...
Les graphiques de la plateforme étaient construits autour d'une palette de couleurs sémantique liée aux catégories de retours (vert pour les éloges, jaune pour les problèmes, bleu pour les suggestions, violet clair pour les questions, gris pour les neutres). Ces couleurs portaient du sens. Quand nous avons ajouté des types de graphiques comme les séries temporelles, où les couleurs représentent des séries de données plutôt que des catégories de retours, cette palette ne pouvait pas être réutilisée ; les associations auraient été trompeuses, et cinq couleurs n'étaient de toute façon pas suffisantes. Nous avions besoin d'une palette catégorielle capable de distinguer jusqu'à 20 séries clairement, sans signification implicite.

J'ai suivi de près la méthodologie de séquence de couleurs inclusives d'IBM, générant des teintes pour chaque nuance avec Colorbox.io, travaillant la palette complète dans Figma, puis adaptant et étendant notre thème MUI pour l'implémenter de manière systématique. Chaque couleur a été conçue pour satisfaire un contraste de 3:1 sur fond clair.
...
J'étais fortement impliqué dans Focus Recommendations en tant que fonctionnalité autonome. Le widget tableau de bord était la pièce qui l'a intégrée dans le produit plus large, traduisant le graphique des facteurs à quatre quadrants et les listes d'insights actionnables pour qu'il fonctionne à la fois sur la page Focus Recommendations dédiée et comme widget redimensionnable aux côtés d'autres contenus du tableau de bord. Adapter la logique de filtre de la fonctionnalité pour coexister avec l'architecture de filtre du tableau de bord nécessitait une collaboration étroite avec le développement pour cartographier les différences et trouver un point d'intégration propre.
...
Le tableau de bord a été mis en production au T3-T4 2021 et est devenu le point d'entrée principal de l'application et la fonctionnalité phare des démos clients. Il a été cité comme facteur clé dans la conversion et le renouvellement de plusieurs clients, des enjeux significatifs pour une petite entreprise où chaque contrat comptait.
