...
featureImage: '/images/dashboard/dashboard-feature.png'
...
_[Company name], AI text analytics for the CX industry_
...
<!-- ASSET: Hero image or short screen recording of the full dashboard. Most important visual in the case study; place before any body text. -->
...
<!-- ASSET: Screenshot of the old static table or fixed chart view; establishes the before state and sets up the contrast with the hero image. -->
...
<!-- ASSET: Short video or GIF (10-15 seconds) showing the layout editor in use: drag to reposition, resize, edit mode toggling. Motion is essential here; a screenshot will not convey the interaction. -->

<!-- ASSET: Widget interaction states spec (the Figma frame showing base, edit, resize X/Y, hover drag, and dragging states). Shows the design thinking behind what the video demonstrates. -->
...
I followed IBM inclusive color sequence methodology closely, generating tints for each hue using Colorbox.io, working out the full palette in Figma, then adapting and extending our MUI theme to implement it systematically. Each color was designed to meet 3:1 contrast against light backgrounds. Contrast between colors was handled structurally: the grid was organized using a chevron skip so that only hues with sufficient contrast between them were adjacent in sequence, and a usage rule was established to always iterate through the palette in order rather than picking colors freely. The accessibility guarantees are built into the system; they do not depend on individual judgment calls at implementation time.

<!-- ASSET: The categorical color palette: all 20 colors, ideally shown in the context of a real chart, or as a Figma frame showing the palette grid or chevron skip structure. -->
...
I was heavily involved in Focus Recommendations as a standalone feature. The dashboard widget was the piece that brought it into the broader product, translating the four-quadrant driver chart so it worked both on the dedicated Focus Recommendations page and as a resizable widget alongside other dashboard content. Adapting the feature filter logic to coexist with the dashboard filter architecture required close collaboration with development to map the differences and find a clean integration point.

<!-- ASSET: The four-quadrant driver chart shown in two contexts side by side: full-page Focus Recommendations view and as a resized dashboard widget. The size adaptation is the core challenge described in the text. -->

_Focus Recommendations is covered in depth as a [separate case study ->](#)._
...
<!-- ASSET (optional): Side-by-side of a launch-era dashboard and a later one showing the richer widget set: shows product maturity without needing to explain it in prose. -->
